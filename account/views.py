    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            if not user.is_admin:
                try:
                    worker = Worker.objects.get(login_id=user)
                except Worker.DoesNotExist:
                    messages.info(request, "Please add information about you in your profile below before proceeding")
                    return redirect("account:profile")
            else:
                try:
                    farmer = Farmer.objects.get(login_id=user)
...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f'Welcome, Account created successfully for {username}. Please Login')
            # redirect to login
            return redirect('account:login')
        else:
